...
import { events as demoEvents } from "@/lib/demo";
...
export default function EventsPage() {
  const [events, setEvents] = useState<LiveEvent[]>(demoEvents.map((event) => ({ id: event.id, title: event.title, startsAt: `${event.dateLabel} ${event.timeLabel}`, location: event.location, format: event.format, playerCap: event.totalSpots, going: event.totalSpots - event.spotsLeft, group: event.group })));
  const [creating, setCreating] = useState(false);
  const [notice, setNotice] = useState("");
  const [form, setForm] = useState({ title: "", startsAt: "", location: "", playerCap: "16", format: "DOUBLES" });

  useEffect(() => { fetch("/api/events").then((response) => response.ok ? response.json() : null).then((data) => data?.events && setEvents(data.events)); }, []);
