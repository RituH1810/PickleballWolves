...
export async function GET(_request: Request, context: { params: Promise<{ roundRobinId: string }> }) {
  ...
  return NextResponse.json({ roundRobin: { ...roundRobin, isOwner: user ? roundRobin.createdById === user.id : false, rounds: roundRobin.rounds.map((round) => ({ ...round, matches: round.matches.map((match) => ({ ...match, players: match.players.map((player) => ({ ...player, name: player.user.name })), scores: match.scores.map((score) => ({ ...score })) })) })) } });
}
